...
export function PricingJsonLd() {
  ...
  const allOffers = [
    ...subscriptions.map((sub) => ({
      ...
      priceCurrency: 'EUR',
      description: `${sub.description} - ${sub.stories} stories per month subscription`,
      priceValidUntil,
      availability: 'https://schema.org/InStock',
      url: `${siteDetails.siteUrl}pricing`,
    })),
  ...
  const pricingSchema = {
    ...
    offers: {
      '@type': 'AggregateOffer',
      lowPrice: Math.min(...[...subscriptions, ...creditPacks].map((p) => p.price)).toString(),
      highPrice: Math.max(...[...subscriptions, ...creditPacks].map((p) => p.price)).toString(),
      priceCurrency: 'EUR',
